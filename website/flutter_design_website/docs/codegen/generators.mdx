...
A single component section called `Anatomy` will be created for the default public constructor of the class. The section contains a `ViewerWidgetBuilder` which contains 
...
the provided viewer initial values, via `viewerInitValueMap` or the default value on the constructor.
